The process of acquiring images ranges from snapping a single image to recording overnight (or longer) time lapses of samples from N different angles. See [[#Acquisition Controls|Acquisition Controls]] for detailed descriptions of the controls.

== Acquiring a Single Image ==
To acquire a single image using the OpenSPIM plugin:
# Navigate the 4D stage to the location you want to image (see [[Operation]] for details).
# Set Stack Depth to 0. This will limit the position to a single plane. (You should not need to change the Step Size.)
# Click ''Stack at this Z plus:'' to add this plane to the position list.
# You may specify an Output directory if you would prefer the image be written straight to disk as an OME-TIFF, rather than opened in Fiji.
## The other options have no effect for a single plane, with the exception of Snap Continuously (which should be unchecked).
# Finally, click ''Oh Snap!'' to take the image.

== Acquiring a Stack ==

Acquiring a single stack is very similar to acquiring a single position.
# Begin by navigating to the place the stack should begin.
# Specify the Stack Depth in μm, and the Step Size (also in μm).
# Click ''Stack at this Z plus:'' to add the stack to the positions list.
# Configure whichever settings are desired:
## The ''Set Z-stage velocity'' and Z settle time controls may be useful for reducing jitter.
## Snap Continuously can be used if recording at the precise locations specified for your stack is of less importance than acquiring images constantly.
## Update Live View will display each slice in the Live Window as it is acquired (allowing you to abort the acquisition early if necessary).
## Lase Full Stack is not generally necessary when recording a single stack, as the reduced time between slices is negligible.
## Though a single stack can easily fit into memory, it is recommended that you specify an Output directory so the stack is saved in OME-TIFF format.
## Using Asynchronous Output is optional, though it will reduce the time spent acquiring considerably.
# Click ''Oh Snap!'' to acquire the stack.

== Single-Plane Time Lapse ==

To acquire a time lapse of a single plane, set up the recording exactly as if you were going to record only [[#Acquiring a Single Image|one image]].
# Enable the Time box and specify an Interval (0 seconds is allowed) and a Count. 
# Optionally specify an Output directory.
# Click ''Oh Snap!'' to begin the time-lapse.

== Multi-View Imaging of a Fixed Sample ==

To record multiple views of a fixed sample:
# For each view:
## Navigates the 4D stage to the location you wish to record.
## Specify the Stack Depth and Step Size.
## Click ''Stack at this Z plus:'' to mark that location.
# Choose the appropriate options:
## The ''Set z-stage velocity'' and Z settle time controls are useful for reducing jitter.
## Snap Continuously will record the stacks as fast as possible, rather than at precisely the given locations.
## Update Live View will show each slice of each stack as it is taken.
## Lase Full Stack can reduce acquisition time.
## Choose an Output directory (highly recommended as these data sets can become quite large).
## Asynchronous Output is useful, but not required for fixed samples.
## If your stacks are particularly far apart and your motor velocity is low, you may wish to override the Device Timeout.
# Click ''Oh Snap!'' to begin the multi-view imaging.

== Multi-View Time Lapses ==

The acquisition tab makes recording a time lapse of any number of views simple: Simply set up a multi-view acquisition as described [[#Multi-View Imaging of a Fixed Sample|above]], and then:
# Optionally enable Anti-Drift (long-term multi-view time lapses are where anti-drift truly shines).
# Enable the time controls, then specify an interval and count.
# Once everything is set up, click ''Oh Snap!'' to begin the acquisition.

== Acquisition Controls ==
[[Image:spimgui-acquiring-annotated.png|480px|right]] 

;(1) Positions List
:This table shows the list of stacks of images to acquire. Each timepoint will consist of the images recorded at each position in this list (as well as the Z slices described by the MATLAB range in the final column).
...
;(18) More Options
:Clicking the ''Show Dialog'' button will open the Acquisition Options dialog below the main window. This window contains additional options which may be desired, but are less commonly-used than most others.
;(19) Oh Snap!
