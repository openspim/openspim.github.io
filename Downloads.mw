...
This is a [http://fiji.sc/ Fiji] package updated with the OpenSPIM [http://fiji.sc/Update_Sites update site]: ''http://openspim.org/update/'', as of April 27th, 2015:

[http://openspim.org/downloads/OpenSPIM-20150426.zip OpenSPIM-20150426.zip (32-bit)] 
[http://openspim.org/downloads/OpenSPIM-20150427.zip OpenSPIM-20150427.zip (64-bit)] 

Installation:
# Download and unpack the right (32bit/64bit) OpenSPIM-... .zip 
# Launch Fiji: in the ''OpenSPIM.app/'' folder, open ''ImageJ-win32.exe'' (or ''-win64.exe'').
# Update Fiji (and OpenSPIM & µManager): ''Help>Update Fiji'' (or simply choose ''Yes, please'' if you are asked to update).
# Install the camera driver if needed (see below).
# Run Micro-Manager: ''Plugins>Micro-Manager>Micro-Manager Studio''.

'''Note:''' if you are interested in building the software from scratch so you can modify it to your heart's extent, please have a look at [[How_to_build_the_software#The_easy_way|this page]].
...
Theoretically, OpenSPIM works with all cameras supported by [http://micro-manager.org/ µManager]. However, OpenSPIM version 1.0 refers to the setup that has a [http://www.hamamatsu.com/eu/en/community/life_science_camera/index.html Hamamatsu ORCA] camera. For such a setup, you need to [http://www.dcamapi.com/ download and install the DCAM driver].
...
[https://docs.google.com/presentation/d/1DvMwhXPf1B4sVwhiWMeYxFYbZHNo7w0F9PMwoCVIO_o/edit?usp=sharing Hardware configuration protocol with screengrabs]
...
# Finally, from the PicardStage category, add in turn the twister, Z stage, and XY stage devices. The software will attempt to guess the correct serial numbers for the motors; these should be verified and corrected as necessary.
