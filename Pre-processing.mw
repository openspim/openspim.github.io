...
[[File:Script editor screenshot.jpg|thumb|400px|right|Screenshot of Fiji Script Editor with re-saving ImageJ macro]]
...
The easiest way to run the macro is to launch the ''Script Editor'' plugin (press letter '''l''' on the keyboard to launch Fiji [http://fiji.sc/Using_the_Command_Launcher Command Finder] and then type Script Editor, then click on ''RUN''). Paste the macro in the Script Editor. Select ImageJ macro from Language menu - this will result in helpful syntax highlighting. If necessary modify the numbers in the loops to reflect the '''input/output directory''', the number of time-points ('''t''') and angles ('''a''') in your time series. Save the macro or simply run it.
...
The end result of running the macro will be 55 new files (11 time-points x 5 angles) in the directory ''tiffs/''. These are regular tif files that we can use for [[Registration|'''bead based registration''']] in Fiji.
