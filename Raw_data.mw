...
[http://openspim.org/downloads/OpenSPIM_Drosophila_11tp.ome.tiff.tar.gz '''DOWNLOAD'''] the ''2.3 GB'' zipped tar archive.

The tarball can be simply expanded into a working directory using 

 tar -xvzf OpenSPIM_Drosophila_10tp.ome.tiff.tar.gz

Each view consists of a stack of 51 images (imaged planes through the specimen 6 microns apart) saved as '''<nowiki>spim_TL<tt>_Angle<a>.ome.tiff</nowiki>''' where '''tt''' refers to zero padded time-points (01 - 235) and '''a''' refers to the five acquisition angles (0-4).

[[File:Montage maxproj tp0 5 10.jpg|thumb|400px|right|Max intensity projections 5 views (columns) from 3 timepoints (0,5,10 rows) from sample OpenSPIM Dresden data]]
...
Now that we have some sample data we can continue on in the SPIMage processing pipeline to the, hopefully temporary, [[Pre-processing|'''pre-processing''']] step.
