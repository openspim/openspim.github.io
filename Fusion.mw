...
|-
|
Next we will specify some new parameters for fusion. We will use the individual registration beacuse that is the only one we have (unless we have run [[Timelapse_Registration|'''time series registration''']] before). We want to fuse into a single output image and we will this time process '''two views in parallel''' (On our super-duper computer we have enough memory to do them all at once, but it is not a realistic situation). 


Click the '''Blending''' checkbox to turn on blending of intensities at the borders between views (otherwise there would be a visible line where one view abruptly ends).


Select '''Content based weigths (fast, approximate)''' over the full ''Content based weights'', the difference is negligible and this will compute significantly faster. 


This time we will '''not downsample''' at all - 1 or 0 both signify this.


Next we fill specify the cropping parameters. Alas these numbers on the screenshot do not look familiar. This is because we collected numbers in the previous [[Fusion#Cropping|'''cropping''']] section on '''four times downsampled''' data. Therefore we have to multiply these number by a factor of '''four''' like this:


* offset x : 109 x 4 = 436
* offset y : 53 x 4 = 212
* offset z : 120 x 4 = 480
* size x   : 138 x 4 = 552
* size y   : 172 x 4 = 688
* size z   : (240-120) x 4 = 480    (note that for z we substract the bottom z-index 120 from the top z index 240)


Finally we decide to '''save the data one directory per time-point''' and click '''OK''' to start processing.
| colspan="2"|
[[File:Final_run1.png|thumb|400px|Screenshot of main fusion dialog with fusion parameters enabled and crop area filled, no downsampling]]
|}
...
[[File:Montage fusion.png|thumb|left|600px|Slices through the fused full resolution output volume (step 80)]]

There are other ways to look at the data as we will see in the [[Browsing|'''browsing''']] section. Note that the fused data are looking perhaps slightly worse compared to the raw data. This is primarily caused by two factors. First the content based fusion combined sharp and blurred information from different views by entropy based weightening and this is not perfect (see the next section [[Fusion#Deconvolution|'''deconvolution''']] for a more advanced fusion approach). Second the embyro is alive and it moves, particularly at this stage of development (gastrulation) the movement is quite fast and it is not quite matched by the acquisition speed of OpenSPIM. I.e. by the time we acquire the fifth view that overlaps with the first views the cells may have moved. The only way to overcome that is to build a faster microscope or slow development down by lowering temperature.

Anyway, this is the final step of [[Fusion#Content_based_fusion|content based fusion]] of a single time-point. From here you can either continue to explore the deconvolution based fusion (right below) or you should switch to [[Timelapse_Registration|'''time series registration''']] which is the next step of the SPIMage processing pipeline.

= Deconvolution =

The description of multi-view deconvolution plugin is available [http://fiji.sc/Multi-View_Deconvolution here]. Deconvolution is still work in progress. Although it is already available in Fiji, it has not yet been published. I will make a detailed tutorial for it as soon as I find the time.
